	...
	/* Checks the Tablets type whether it is microsft or apple or samsung */
		...
		else 
		{
			...
			{	
				...
				{
				  if(entry.getValue().getRetailer() != null && entry.getValue().getRetailer().equals("Apple"))
				  {
			...
			else if (CategoryName.equals("Microsoft"))
			{
				for(Map.Entry<String,Laptops> entry : SaxParserDataStore.laptops.entrySet())
				{
				  if(entry.getValue().getRetailer() != null && entry.getValue().getRetailer().equals("Microsoft"))
				  {
					 hm.put(entry.getValue().getId(),entry.getValue());
				  }
				}
			...
			else if (CategoryName.equals("ASUS")) 
			{
				for(Map.Entry<String,Laptops> entry : SaxParserDataStore.laptops.entrySet())
				{
				  if(entry.getValue().getRetailer() != null && entry.getValue().getRetailer().equals("ASUS"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}	
		...
		if(userType.equalsIgnoreCase("storemanager")) {
			pw.print("<li><form method='get' action='Addlaptops'>"+"<input type='hidden' name='name' value='abe'>"+
		
				"<input type='hidden' name='type' value='laptops'>"+
				"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
				"<input type='hidden' name='access' value=''>"+
			    "<input type='submit' value='Add/Update' class='btnreview'></input></form></li>");
			pw.print("<li><form method='get' action='Deletelaptops'>"+"<input type='hidden' name='name' value='abe'>"+

...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    if(userType.equalsIgnoreCase("customer")) {
			if (request.getParameter("Order") == null) {
				pw.print(
						"<table align='center'><tr><td>Enter OrderNo &nbsp&nbsp<input name='orderId' type='text'></td>");
...
//	    	pw.print("<li><form method='get' action='AddUpdate'>"+"<input type='hidden' name='name' value='abe'>"+
				  ...
				  // if(oi.getOrderName().equals(orderName) && oi.getUserName().equals(username)) {
					for(Map.Entry<Integer, ArrayList<OrderPayment>> entry : orderPayments.entrySet()) {
						for (OrderPayment oi : entry.getValue()) {
							if (oi.getOrderId() == orderId && oi.getOrderName().equals(orderName)) {
								ListOrderPayment.add(oi);
								pw.print("<h4 style='color:red'>Your Order is Cancelled</h4>");
							}	
						}

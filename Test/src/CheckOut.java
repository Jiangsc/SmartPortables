...
//	 			session.setAttribute("login_msg", "Please Login to add orders!");
...
public class CheckOut extends HttpServlet {
	...
	protected void storeOrders(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		pw.print("<table  class='gridtable'><tr><td>Customer Name:</td><td>");
		if(userType.equalsIgnoreCase("salesman")) {
			pw.print("<input type='text' name='customerName'>");
		} else {
			pw.print("<input type='text' name='customerName' value='"+userName+"' readonly='readonly'>");
		}
		//pw.print(userName);
		pw.print("</td></tr>");
		// for each order iterate and display the order name price
